...
export default function ShareButtons({ title, slug, variant = 'inline' }: ShareButtonsProps) {
  const [copied, setCopied] = useState(false)
  const url = `https://www.indianpotato.in/samachar/${slug}`
  ...
  return (
    <div style={{ display: 'flex', gap: 8, alignItems: 'center', justifyContent: variant === 'bar' ? 'center' : 'flex-start', flexWrap: 'wrap' }}>
      {buttons.map((btn) => (
        <a
          key={btn.label}
          href={btn.href}
          ...
          style={{
            ...
            background: btn.bg, textDecoration: 'none',
      ...
      <button
        onClick={copyLink}
        aria-label={copied ? 'लिंक कॉपी हो गया' : 'लिंक कॉपी करें'}
        style={{
          display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
          width: SIZE, height: SIZE, borderRadius: '50%',
          background: copied ? '#05420d' : '#666', border: 'none', cursor: 'pointer',
      ...
      >
        {copied ? (
          <svg width="16" height="16" viewBox="0 0 24 24" fill="white" stroke="white" strokeWidth="2"><path d="M20 6L9 17l-5-5"/></svg>
        ) : (
